...
public class T0Calib extends AnalysisMonitor{
        ...
        for (int i = 0; i < nsec; i++)
        {
            for (int j = 0; j < nsl; j++)
            {
                for (int k = 0; k < nSlots7; k++)
                {
                    ...
                    {
                        hNm = String.format("timeS%dS%dS%dCbl%d", i + 1, j + 1, k + 1, l + 1);//Hamza
                        TDCHis.put(new Coordinate(i,j,k, l), new H1F(hNm, 150, tLow4T0Fits[j], tHigh4T0Fits[j])); 
                                                                                                                                                                                        // HBHits
                        hTtl = String.format("time (Sec%d SL%d Slot%d Cable%d)", i + 1, j + 1, k + 1, l + 1);//Hamza
    ...
    private double[] getT0(int i, int j, int k, int l) {
        ...
        FitFunc.setParameter(0, h.getMax());
       ...
       // DataFitter.fit(FitFunc, h, "Q"); //fit to the new fit function
...
//        }
        ...
        if(gr.getDataSize(0)>1 && histRangeIntegl>50) {
            ...
            double T0 = n/d;
            double T0n =FitFunc.getParameter(0)/FitFunc.getParameter(1); //new T0 frm new fit function
